...
#include "C:\Data\3D Modelling\Learning\Metallic Textures.inc"
#include "C:\Data\3D Modelling\Learning\Gear.inc"
...
// Icon for Macro_Arch function
#declare BoltRadius = 0.1;
#declare LeftX = -5;
#declare RightX = 5;

#declare Icon_Macro_Arch =
  object
    { #declare X = LeftX;
...
#macro Disc(P)
  union
      { cylinder { <0, -D / 2, 0>, <0, D / 2, 0>, R texture { pigment { P } } }
        torus { R, D texture { T_Silver_1A } }
      }
#end
...
#declare Icon_Group =
  ...
  object
    { union
        { object { Disc(Red) translate <L / 2, 0, L * sqrt(3) / 2> }
          object { Disc(Green) translate <L, D, 0> }
          object { Disc(Blue) translate <0, -D, 0 > }
        }
  	    rotate x * -90
        //translate <-0.6, -0.5, 0>
  	    rotate y * 15
  	    scale 1.25
  	    translate <1.25, 0.85, 0>
    }
...
#macro Bolt(X, Y, F)
  cylinder
    { < X, Y, -BoltRadius >, < X, Y, BoltRadius >, BoltRadius
      #if (F)
        texture { T_Chrome_1A } }
      #else
...
#declare Thing =
  union
    { cylinder { <-1, -0.5, 0>, <0.5, 0.5, 0>, BoltRadius texture { T_Silver_1A } }
      cylinder { <-1, -0.5, 0>, <-0.5, 2.5, 0>, BoltRadius texture { T_Silver_1A } }
      cylinder { <-0.5, 2.5, 0>, <0.5, 0.5, 0>, BoltRadius texture { T_Silver_1A } }
  ...
  union
    { intersection
        { torus
          ...
          plane { -y, 0 rotate z * A}
          plane { -y, 0 rotate z * -A}
        }
      
      cone { < 0, R, 0>, BoltRadius * 1.5, <-R / 2, R, 0>, 0 rotate z * A }
      cone { < 0, R, 0>, BoltRadius * 1.5, <+R / 2, R, 0>, 0 rotate z * -A }
      
      texture { pigment { colour Yellow + White / 2 } }
    }
...
#declare Icon_FlipX =
  union
    { object { Thing                translate < -1, 1, 0 > }
      object { Thing rotate y * 180 translate < +1, 1, 0 > }
      
      object { Divider }
      
      object { Arrow translate < 0, -1.25, 0 > }
...
#declare Icon_FlipY =
  union
    { object { Thing                translate < -1, 1, 0 > }
      ...
      object { Arrow rotate z * 180 translate < 0, 0.75, 0 > }
...
#declare Icon_Rotate =
  object
    { cylinder { <0, 0, 0>, <1, 1, 1>, 0.1 texture { T_Silver_1A } }
      scale 1
      translate < 5, 0, 0 >
    }
...
#declare Icon_Test =
  union
    { #local N = 18;
      object { Gear(10, N, 0.55, clock * 2 / 6) }
      object
        { Gear(20, N * 2, 0.55, -clock / 6)
          rotate z * (30 + 180 / N)
          translate <30, 0, 0.1>
          rotate z * 30
        }
...
#switch (iMode)
  // Normal
  #case (0)
    plane { -z, -20 texture { pigment { Gray25 + Blue / 5} } }
  #break
  
  // Selected
  #case (1)
    plane { -z, -20 texture { pigment { Green * 0.5} } }
  #break
  
  // Disabled
  #case (2)
    plane { -z, -20 texture { pigment { Red * 0.5} } }
    object
      { union
          { cylinder { P1, P3, dW * 2}
            cylinder { P2, P4, dW * 2}
  ...
  #case (3)
    plane { -z, -20 texture { checker texture { T_Silver_1A } texture { T_Chrome_1A } } }
  #break  
#end
